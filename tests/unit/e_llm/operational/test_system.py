...
from e_llm.operational.system import CpuInfo, DiskInfo, RamInfo, SystemEvaluator, SystemInfo

##### DATA MODELS #####


async def test_cpu_info_frozen() -> None:
    info = CpuInfo(model="Test", cores_physical=4, cores_logical=8, frequency_mhz=3600.0)
...
async def test_gpu_info_optional() -> None:
    info = SystemInfo(
        cpu=CpuInfo(model="Test", cores_physical=4, cores_logical=8, frequency_mhz=3600.0),
